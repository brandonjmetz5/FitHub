    ...
    var body: some View {
        ZStack {
            ...
            VStack(spacing: 0) {
                ...
                // MARK: — Daily Targets Card (uses updated daily allowance)
                if let profile = profileViewModel.profile {
                    DailyTargetsCard(
                        totalCalories: Double(viewModel.totalCalories),
                        calorieTarget: viewModel.dailyCalorieAllowance,
                        totalProtein: Double(viewModel.totalProtein),
                        proteinTarget: profile.proteinTarget,
                        totalCarbs: Double(viewModel.totalCarbs),
                        carbsTarget: profile.carbsTarget,
                        totalFat: Double(viewModel.totalFat),
...
fileprivate struct DailyTargetsCard: View {
    let totalCalories: Double, calorieTarget: Double
    let totalProtein: Double, proteinTarget: Double
    let totalCarbs: Double, carbsTarget: Double
    let totalFat: Double, fatTarget: Double
...
fileprivate struct MetricProgressRow: View {
    let title: String, current: Double, target: Double, unit: String

    var progress: Double { current / target }
